// Complete seed script for MongoDB - can be run directly with mongosh

db = db.getSiblingDB('talentsync');
...
// Generate companies
function generateCompanyRecord(template, index) {
  ...
  return {
    ...
    LeadInvestors: template.LeadInvestors,
...
const companies = [];
for (let i = 0; i < 50; i++) {
  companies.push(generateCompanyRecord(companyTemplates[i % companyTemplates.length], i));
}

db.companies.insertMany(companies);
print('Inserted ' + companies.length + ' companies');
...
const jobTemplates = [
  {
    ...
    description: 'Build responsive web applications with modern frameworks while collaborating closely with product and design.',
    baseRating: 4.4,
  ...
  },
  {
    ...
    description: 'Design predictive models, partner with engineering to productionize insights, and coach teams on data best practices.',
    baseRating: 4.6,
    size: '201 to 500 employees',
    ownership: 'Private',
    industry: 'Software',
  ...
  },
  {
    ...
    description: 'Define product strategy, prioritize roadmaps, and orchestrate cross-functional execution with measurable outcomes.',
    baseRating: 4.5,
  ...
  },
  {
    ...
    description: 'Analyze renewable energy deployments, identify efficiency gains, and partner with operations on field execution.',
    baseRating: 4.2,
  ...
  },
  {
    ...
    description: 'Automate deployment pipelines, monitor cloud infrastructure, and ensure high availability for mission-critical systems.',
    baseRating: 4.3,
    size: '201 to 500 employees',
    ownership: 'Private',
    industry: 'Software',
...
for (let i = 0; i < 50; i++) {
  ...
  jobs.push({
    ...
    'Job Description': template.description + ' Opportunity batch #' + (i + 1) + '.',
    Rating: Number((template.baseRating + (i % 3) * 0.1).toFixed(1)),
...
print('Inserted ' + jobs.length + ' jobs');

// Salaries
const firstNames = ['Alex', 'Jordan', 'Taylor', 'Casey', 'Morgan', 'Riley', 'Sydney', 'Harper', 'Parker', 'Quinn', 'Reese', 'Dakota', 'Emerson', 'Hayden', 'Rowan', 'Sawyer', 'Tatum', 'Blair', 'Dakari', 'Elliott'];
const lastNames = ['Johnson', 'Garcia', 'Patel', 'Nguyen', 'Brooks', 'Santiago', 'Rivera', 'Hughes', 'Foster', 'Khan', 'Walker', 'Armstrong', 'Ali', 'Hernandez', 'Diaz', 'Singh', 'Kim', 'Ochoa', 'Holland', 'Nakamura'];

const salaries = [];
for (let i = 0; i < 40; i++) {
  const company = companies[i % companies.length];
  const jobTemplate = jobTemplates[i % jobTemplates.length];
  const basePay = 72000 + (i % 10) * 4500 + Math.floor(i / 10) * 6000;
  const overtimePay = (i % 4) * 1200;
  const otherPay = (i % 5) * 950;
  const totalPay = basePay + overtimePay + otherPay;
  const totalPayBenefits = totalPay + 18000 + (i % 3) * 2500;
...
print('Inserted ' + salaries.length + ' salaries');
...
db.jobapplications.insertMany(jobApplications);
print('Inserted ' + jobApplications.length + ' job applications');
...
print('- Salaries: ' + salaries.length);
print('- Job Applications: ' + jobApplications.length);
